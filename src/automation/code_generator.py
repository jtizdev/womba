...
ASYNC version - all subprocess calls are non-blocking.
"""
...
async def _run_command(cmd: list, cwd: Path, timeout: int = 300, check: bool = False) -> tuple:
    """
...
class TestCodeGenerator:
    ...
    async def generate_code(
        ...
        # Step 4: Create branch and commit (async)
        if not await self.pr_creator.create_branch_async(target_branch):
            logger.error("Failed to create branch")
            return []

        commit_message = self._build_commit_message(test_plan)
        if not await self.pr_creator.commit_files_async(generated_files, commit_message):
            logger.error("Failed to commit files")
            return []

        # Step 5: Push branch (async)
        if not await self.pr_creator.push_branch_async(target_branch):
            logger.error("Failed to push branch")
            return []

        # Step 6: Create PR if requested (async)
        if create_pr:
            pr_url = await self.pr_creator.create_pr_async(
                test_plan=test_plan,
...
""")
    ...
    async def _generate_with_aider(self, prompt: str) -> List[str]:
        """
        Generate code using aider in customer's repo (async).
        ...
        try:
            ...
            aider_cmd = ["aider"]
            try:
                await _run_command(["aider", "--version"], cwd=self.repo_path, timeout=10)
    ...
    async def _generate_with_cursor(self, prompt: str) -> List[str]:
        """
        Generate code using cursor-cli in customer's repo (async).
        ...
        try:
            ...
            if returncode != 0:
                logger.error(f"cursor-cli failed: {stderr}")
            ...
            if not generated_files:
                _, git_stdout, _ = await _run_command(
                    ["git", "status", "--porcelain"],
                    cwd=self.repo_path
                )
                for line in git_stdout.split('\n'):
                    if line.strip():
